...
class ProgrammingAssignment1 {
   ...
   static int[] quickSort(int[] data, int n){
      ...
      //r -> starting index, p -> ending index
      return quickSortDriver(data, 0, n-1);
      
   }

   static int[] quickSortDriver(int[] data, int p, int r) {
      if(p < r) {
         int q = partition(data, p, r);

         quickSortDriver(data, p, q-1);
         quickSortDriver(data, q+1, r);
      
      }
      return data;
   }

   //Swap for quick sort: swap a and b in the array data
   ...
   //Partition the array
   //Takes the last element as the pivot and then puts it in
   //the correct position
   //All smaller elements go on left, greater on right
   static int partition(int[] data, int p, int r) {
      //choose pivot
      int x = data[r];

      //counter on right of pivot
      int i = (p - 1);
   ...
   static int[] insertionSort(int[] data, int n){
	...
	public static void main(String[] args) {
      ...
      if(!success){
         ...
         long nanoseconds = totalTime[medianof3(totalTime[0], totalTime[1], totalTime[2]) - 1];
         long miliseconds = nanoseconds/1000000;
         System.out.println("Or " + miliseconds + "ms");
      }
	}
}
